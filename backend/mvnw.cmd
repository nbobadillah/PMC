...
if not "%JAVAC_PATH%" == "" goto DeriveJavaHome
...
:DeriveJavaHome
for %%i in ("%JAVAC_PATH%") do set "JAVA_BIN_DIR=%%~dpi"
...
if exist "%JAVA_HOME%\bin\java.exe" goto init
for /D %%i in ("C:\Program Files\Java\jdk-2*") do if exist "%%~fi\bin\java.exe" set "JAVA_HOME=%%~fi"
if exist "%JAVA_HOME%\bin\java.exe" goto init
